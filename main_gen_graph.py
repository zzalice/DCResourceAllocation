...
if __name__ == '__main__':
    ...
    # IterateAlgo(iteration=i, folder_data=f_data).iter_layer(l)
    ...
    # IterateAlgo(iteration=i, folder_data=f_data).iter_ue(ut)
    ...
    # IterateAlgo(iteration=i, folder_data=f_data).iter_due_to_all(p_due)
    ...
    # IterateAlgo(iteration=i, folder_data=f_data).iter_gnb_bw(gnb_bw)

    # ---Graphs for co-channel bandwidth---
    cochannel_bw = [j for j in range(5, 51, 5)]  # <-- change
    # IterateAlgo(iteration=i, folder_data=f_data).iter_cochannel(cochannel_bw)
